...
def _apply_sign(raw_label: str, value: float | None) -> float | None:
    ...
    - **손실만 명시된 라벨**("영업손실"/"매출총손실"/"당기순손실" — "이익"이 없는
      ...
      다시 뒤집어 양수(이익)로 저장한다.
    - **"이익(손실)" 조합형 라벨**(회사가 흑자든 적자든 같은 줄을 재사용하는
      템플릿 — "영업이익(손실)"/"당기순이익(손실)" 등): 원문에 부호가 이미
      정확히 반영돼 있다(양수=이익, 괄호=음수=손실) — 절대 뒤집지 않는다.
      실측(EUC-KR 원문 20220127000408) "영업이익(손실)" 행이 "(6,308,961,098)"로
      이미 음수 표기돼 있는데, 예전 코드는 `value > 0`일 때만 반전하는
      조건이라 이 경우는 우연히 맞았지만, 같은 라벨에 실제로는 흑자라 원문이
      양수로 찍힌 경우까지 상정하면 예전 코드는 그 흑자를 손실로 잘못
      반전시키는 잠재 버그가 있었다 — 라벨에 "이익"이 있는지로 갈래를 나눠
      두 사례 모두 정확히 처리한다.
    """
    if value is None:
        return None
    if "손실" not in raw_label:
        return value
    if "이익" in raw_label:
        return value  # 조합형 라벨은 원문 부호를 그대로 신뢰한다.
    return -value  # 손실만 명시된 라벨은 부호와 무관하게 항상 반전한다.
