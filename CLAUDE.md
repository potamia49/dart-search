...
- **"손실" 라벨 부호 처리**: 순수 손실 라벨("영업손실")은 원문이 부호 없이
  양수로 찍혀 있어 항상 반전해야 하고, 조합형 라벨("영업이익(손실)")은 원문
  부호를 그대로 신뢰해야 한다 — 어순(이익/손실 순서)이 달라도 "이익"이
  라벨에 있는지로만 구분한다.
...
- **스키마 확장은 항상 "컬럼 추가 + 소급 재파싱 없음"** 패턴을 따른다(신규
  Phase 2 실행분부터만 채워짐). **현재 소급 재파싱 대기 중인 후보**(전부
  로컬 캐시만 읽으면 되므로 쿼터 0건, 사용자가 원하면 언제든 일괄 처리
  가능): EUC-KR 인코딩 복구분 51건, `noncurrent_assets` 로마숫자 접두어
  버그 수정분, 라벨 정규화 확장으로 정정 가능한 PARTIAL 36건,
  `auditor_name`/`auditor_address` NULL인 기존 결과.
...
pytest tests/ -q                # 2026-07-21 기준 271 passed
```
