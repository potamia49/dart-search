...
- 템플릿은 저장소 루트 `tamplate/audit_proposal_template.html`(폴더명 오타가
  ...
  빠뜨리면 507로 실패한다.** 이 "템플릿 무수정" 관행에는 **예외 3건**이 있다 —
  아래 "자본잠식 회사 등급"·"자본잠식 리스크 문구"·"자본잠식 차트 2종" 항목
  (셋 다 원인이 같다: 자본총계가 음수면 부채비율도 음수가 되는데, 음수는 결측
  가드를 정상값으로 통과한다).
...
- 동종업종/지역 비교군(`peers`/`industryAverage`/`regionGroup`/`opinionSummary`)은
  **데이터 소스가 아직 없어 빈 값**이다 — 생기면 `build_report_payload()`만 고친다.
