...
- 템플릿은 저장소 루트 `tamplate/audit_proposal_template.html`(폴더명 오타가
  ...
  빠뜨리면 507로 실패한다.** 이 "템플릿 무수정" 관행에는 **예외 4건**이 있다 —
...
- **동종업종/지역 비교군(`peers`/`industryAverage`/`regionGroup`)은 "같은 Job에서
  ...
  - **peers**: `induty_code` **소분류(앞 3자리) prefix** 우선, 자기 자신을 뺀 매칭이
    2건 미만이면 **중분류(앞 2자리)** 로 한 번만 폴백(대분류까지 넓히지 않는다). 그래도
    2건 미만이면 `peers=[]`·`industryAverage=null`(에러가 아니라 정상). 실측 코드 길이가
    2~5자리로 제각각이라 prefix 매칭이 유일하게 안전한 방식이다. **감사의견이 의견거절/
    부적정인 회사는 후보에서 통째로 뺀다**(2026-08-04, 아래 항목 참고).
  ...
  - **regionGroup**: 지역 세분화 없이 **같은 Job 전체**가 후보 풀이다(Job의 검색 조건
    자체가 지역이므로). 대상 회사가 항상 배열 첫머리에 `isTarget: true`로 들어가고, 다른
    회사가 2건 미만이면 `[]`. **회사명은 실명 그대로 노출**한다(DART 공시 기반 공개 정보).
    ...
    **76개 대상 문서**의 지역 표에 섞인다). 대상 회사에도 같은 규칙을 적용하되(한 표 안에서
    ...
    사실상 안 보인다** — 배경색 추가는 템플릿 수정이 필요해 이번 범위 밖으로 남겼다.
