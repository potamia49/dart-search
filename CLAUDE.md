...
- **연도별 감사인 + "감사인 변동" 표시**(2026-07-26, dart-backend). 사용자 요청
  ...
  **파서는 여전히 무변경**이고 DB에는 원문 표기 그대로 저장한다. 로컬 캐시 재스캔
  ...
  · 로컬 캐시 영향 스캔(읽기 전용, 소급 반영 **미실행**): 기존
  `financial_snapshots` 4,147행 중 **3,685행이 감사인 이름을 채울 수 있고**
  (448행은 전기 유래라 대상 아님, 14행은 원문에 감사인 없음 — 진성 결측),
  이력 보유 1,159개 회사 기준 최종 판정(보정 1~3 모두 반영)은 **변동 있음 221 /
  변동 없음 746 / 판정 불가 192**(+3건은 감사인 0건)이 될 전망이다. 실제 값은
  신규 Phase 2 실행분부터만 채워진다(소급 재파싱 여부는 사용자 별도 결정 — 이
  프로젝트의 확립된 관행. **DB의 `auditor_name`/`auditor_changed`는 아직 전부
  NULL이라 위 보정 1~3에 해당하는 소급 처리 대상 자체가 없다**. 단
  `results.auditor_name`은 이 기능 이전부터 있던 컬럼이라 예외 — 1,206행 중
  **15행이 [보정 3]의 절단 버그로 이미 손상 저장**돼 있었다(우회계법인→일우회계법인
  6건, 번지태흥공인회계사감사반→태흥공인회계사감사반 1건 등)). `pytest` 314 passed
  (보정 1~3 포함).
