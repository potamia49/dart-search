...
- **IFRS "(첨부)재무제표" 첨부문서 구조**(2026-07-22, 롯데미쓰이화학 rcept
  ...
  **PARTIAL→OK 40건**(FAILED→ 전이 0 — 결과테이블의 FAILED 858건은 전부 rcept
  ...
  필드 0로 확인).)
  · **UI 전용 두 모듈이 이 서식을 인식하지 못하던 drift → 공용 워커로 통합**
  (2026-07-27, 이래CS job 24/result 5503 rcept 20260401004343 사용자 실측 지적).
  파이프라인 파서(`xml_parser.py`)는 2026-07-22에 첨부 서식을 지원했지만
  **원문 섹션 열람(`document_sections.py`)과 세부계정 펼치기(`account_detail.py`)
  에는 그 확장이 반영되지 않아** 서식 인식 기준이 세 곳으로 갈라져 있었다 —
  그래서 같은 회사가 "재무이력 표에는 값이 있는데(=results/snapshots는 정상)
  원문보기는 '원문을 찾을 수 없습니다'이고, 유동자산을 눌러도 세부계정이 비어
  있는" 상태가 됐다(두 모듈 모두 `<TABLE-GROUP><TITLE>`+`ACLASS="FINANCE"`
  조합만 인식). 재구현 대신 **서식 인식 상태기계를 `xml_parser.walk_statement_tables`
  하나로 뽑아 세 모듈이 공유**하게 고쳤다(TITLE 판정·`in_attach` 가드·`<P>`/캡션
  표 제목 감지가 한 곳에만 존재 → 다음 서식 확장이 자동으로 세 곳에 반영된다).
  `parse_xml_financials`는 이 워커를 쓰도록 재작성했고 **로컬 캐시 4,922건 전수
  대조로 결과 바이트 동일(diff 0)** 확인. 표 **안**을 읽는 방식만 서식별로 갈린다:
  ① 첨부 데이터 표는 평범한 `<TD>`라 FINANCE의 `ALEVEL`이 없어 계정 계층을
  **선행 공백(들여쓰기)의 그룹 내 상대 순위**로 판정한다(`account_detail._attach_levels`,
  들여쓰기 폭은 회사마다 1~4칸이라 절대값이 아니라 순위). 들여쓰기를 잃어버린
  원문을 위한 보조 단서로 **하위계정 번호 접두어("1."/"(1)"/"가.")를 대분류
  **경계 판정에만** 쓴다 — 번호를 계층 "깊이"로까지 쓰면 번호 붙은 소계가 번호
  없는 세부 행의 부모인 원문(20250328000629/20260331002166)에서 부모·자식이
  뒤집혀서, 레벨 순위는 끝까지 들여쓰기로만 매긴다. 들여쓰기가 아예 없는 표에서는
  번호 없는 세부 행도 같은 대분류에 이어 붙이되(그러지 않으면 "1./2."만 잡히고
  "3./4."와 세부 행이 빠진 **합계가 안 맞는 일부 목록**이 나온다, 20230321000429),
  **금액이 없는 구역 헤더("자산"/"부채")와 로마숫자 항목번호 행("Ⅳ.현금의 증감")은
  대분류를 닫는다**(20230321000531 실측 가드). ② 요약 필드로 매핑되는 행은
  들여쓰기와 무관하게 항상 대분류로 본다(롯데미쓰이는 "판매비와관리비"를 한 칸
  들여써 적어, 안 그러면 매출총이익의 하위계정으로 붙는다). ③ 부호 함수는 파이프라인과
  같은 `_apply_sign_ifrs`를 호출하지만, **세부계정으로 화면에 실리는 행은 항상
  원문 부호 그대로**다 — cogs/sga로 매핑되는 라벨은 위 ②에 의해 항상 대분류(요약)
  행으로 소비되고 children으로 append되지 않으므로, abs 정규화 분기는 이 경로에서
  실행되지 않는다(dart-qa 2026-07-27 지적으로 주석/문서 문구만 정정 — 동작 오류는
  없음, `_apply_sign_ifrs` 재사용 자체는 유지).
  ④ 원문 섹션 열람은 표를 소비하지 않고 구간 전체를 훑어야 해서
  `detect_caption_while_pending=True` opt-in을 쓰고, 자본변동표 등 다른 재무제표
  제목에서 구간을 끊는다(손익계산서 탭에 자본변동표가 딸려 들어가던 것 방지).
  **기존 FINANCE 경로는 전 구간 무변경.** 로컬 캐시 4,922건 전수 재대조(읽기
  전용, API 0건): `account-detail`은 **228건이 빈 값 → 값**(값이 바뀐 문서 0건,
  잃은 문서 0건), `document-sections`는 **bs 247 / is 246 / cf 246건이 신규로
  열람 가능**해졌고 **기존에 열리던 문서의 HTML이 바뀐 건 0건**. 세부계정 계층
  자체검증(첨부 245건, **children 합계 == 요약 대분류 값** 항등식 — 계층을 잘못
  잡으면 즉시 어긋난다): 재무상태표 4개 대분류 기준 **841건 정확 일치**, 5건은
  원문이 형제 계정의 들여쓰기를 뒤섞어 레벨만 갈린 경우(값은 전부 포착), 나머지
  2건은 파서 문제가 아니다 — 20230407000729는 **원문 자체의 합계 오류**(당기 열만
  4개 항목 합 ≠ 소계, 전기 열은 정확히 일치), 20260403002648은 들여쓰기·번호가
  모두 없는 하위 행 1건(해당 소계의 0.16%)이라 단서가 없다. 번호 보조 단서로
  **50개 문서가 세부계정을 새로 얻었고 잃은 문서는 0건**(대부분 들여쓰기가 없는
  현금흐름표). 현금흐름표 항등식은 526건 일치/61건 불일치인데, 불일치는 전수
  확인 결과 **원문이 소계와 세부 행을 같은 들여쓰기로 적어 둘 다 최상위로 잡히는
  경우**(합이 이중 계상됨)라 값 자체는 원문과 일치한다 — 항등식으로는 검증
  불가능한 서식이지 데이터 오류가 아니다. 회귀 fixtures: 신규 2건
  (**20260401004343** 이래CS — 캡션표+연결+4칸 들여쓰기, 사용자 신고 원문 /
  **20250328000629** 한화컴파운드 — 번호 붙은 소계가 번호 없는 세부의 부모인
  대조군) + 기존 첨부 fixtures 4건(20250324000776/20240329000968/20230322000842/
  20230321000531)을 두 모듈용 테스트로 재사용. `pytest` 334 passed(314 → +20).
  **소급 재파싱은 개념상 해당 없음** — DB 값이 아니라 로컬 캐시 원문을 매 요청마다
  새로 파싱하는 조회 API 로직만 고쳤으므로 기존 Job에서도 즉시 반영된다(배포
  exe에는 다음 재빌드 때 반영).
  · **dart-qa 리뷰(2026-07-27) 결론**: "diff 0"·전수 대조 주장을 독립 재실행으로
  재현 확인, `parse_xml_financials` 리팩터링의 분기별 등가성도 확인. 발견 6건 중
  실동작 오류는 0건(전부 기존 한계가 드러났거나 문서 문구 불일치) — 위 ③ 부호
  주석 정정은 이 지적 반영. 나머지 발견: (a) 포괄손익계산서처럼 손익계산서
  캡션이 문서에 두 번 나오면 두 번째 IS 데이터 표가 `values_cur` 불변으로 안
  소비돼 그 뒤 현금흐름표 캡션까지 "IS 데이터 표"로 잘못 소비되는 기존 파이프라인
  한계(첨부 228건 중 26건, `results.cf_*`는 NULL인데 세부계정에는 CF 항목이
  보이는 UX 불일치) — **사용자가 이번 범위에서는 보류 결정**(별도 요청 시
  `detect_caption_while_pending=True`를 파이프라인에도 적용하는 방안 검토,
  단 저장값이 바뀌므로 신규 실행분부터만 적용할지 소급 재파싱까지 할지 별도
  승인 필요); (b) 들여쓰기·번호가 전혀 없는 표는 세부계정이 회사마다 나오거나
  안 나오는 편차(값 오류 아님, 기존 FINANCE 경로와 동일한 설계상 한계); (c)
  `has_roman_numeral_prefix`가 마침표 없는 로마숫자 헤더는 못 잡음(실측 사례
  없어 규칙 미확장, 이 프로젝트 관행과 일치). 상세개발계획.md에 섞여 있던 무관한
  §4-11(다중 선택 다운로드) 변경은 **건드리지 않고 이번 파서 변경만 커밋**하기로
  결정.
...
- **연도별 감사인 + "감사인 변동" 표시**(2026-07-26, dart-backend). 사용자 요청
  ("결과조회 컬럼에 감사인 변동 여부 / 상세의 감사의견 다음 행에 감사인 정보")에
  대한 **백엔드(DB·파이프라인·API) 부분**이다. CF 4항목·영업외손익 2항목과 동형인
  "컬럼 추가 + 소급 재파싱 없음" 패턴이며 **추가 API 호출 0건**(STEP 7이 이미 여는
  원문·로컬 캐시만 읽는다).
  · `financial_snapshots.auditor_name`(신규 컬럼) — STEP 7이 각 연도 원문을 파싱할
  때 `extract_auditor()`로 함께 뽑아 저장한다. **`from_current_period=1`인 행에만**
  채운다(전기 열 유래 행에 그 공시의 감사인을 적으면 연도-감사인 대응이 어긋난다).
  주소는 연도별로 보관하지 않는다 — 아래 `account-detail`이 원문에서 매번 준다.
  · `results.auditor_changed`(신규 컬럼, **tri-state** 1/0/**NULL**) — 위 연도별
  이름을 회사 단위로 집계한 캐시. 1=서로 다른 감사인 2곳 이상, 0=전부 동일,
  NULL=판정 불가(이름을 확보한 연도 1개 이하). 다른 `excluded_by_*`와 달리 기본값이
  0이 아니라 NULL인 것이 핵심이다 — 기존 완료 Job을 "변동 없음"으로 단정하지 않는다.
  `_run_history_collection`이 회사 1건을 끝낼 때마다 `_update_auditor_change_flag()`로
  갱신한다(resume 조기반환 경로 포함).
  · 비교는 `_auditor_key()`(pipeline.py)로 표기 차이를 흡수한 뒤 한다 — 공백 제거 +
  접미어-선행 표기("회계법인 원지"↔"원지회계법인") 통일 + 이름에 딸려 들어온 서명자
  표기("청현회계법인 대표이사문창규", 실측 3,685건 중 31건/10종 — 글자 사이가 벌어진
  "대 표 이 사"를 파서 `_SIGNER_RE`가 못 끊어 생긴다) 절단. **파서는 무변경**이고
  DB에는 원문 표기 그대로 저장한다. 이 정규화가 없으면 로컬 캐시 실측에서 오탐 3건.
  · API: `GET /results`·`/export`에 `auditor_changed` tri-state 필터 파라미터 추가
  (true/false/미지정, NULL은 양쪽 모두에서 빠짐), `ResultResponse.auditor_changed`,
  `SORTABLE_COLUMNS`+`RESULT_COLUMN_LABELS`("감사인변동여부"),
  `FinancialSnapshotResponse.auditor_name`, **`account-detail` 응답에
  `auditor_name`/`auditor_address` 추가**(그 연도 원문을 그 자리에서 다시 파싱 —
  화면이 감사의견 다음 행에 감사인을 보여주는 데 쓰며, **기존 Job에서도 값이 나온다**).
  · **프론트엔드 반영 완료**(2026-07-26): 결과 목록에 "감사인변동" 컬럼(배지 —
  변동 있음만 강조, 동일/판정불가는 평문)과 필터 탭, 상세 재무이력 표의 "감사의견"
  행 바로 다음 "감사인" 행(연도별 이름, `auditor_changed_from_prev===true`인 연도만
  주황 강조 + "변경" 배지)을 `ResultPage.tsx`/`resultColumns.ts`/
  `ResultDetailDrawer.tsx`가 구현했다. 상세는 서버가 내려주는 `auditor_changed_from_prev`
  값만 읽고 자체 비교 로직을 두지 않는다([보정 2] 참고).
  · **[보정 1] `_auditor_key()`가 접미어 뒤 법인명을 통째로 버리던 버그**
  (2026-07-26 dart-qa 리뷰 → 수정): 옛 구현은 접미어("회계법인")가 문자열 시작이
  아니기만 하면(`rfind(...) > 0`) 그 뒤를 전부 서명자로 보고 잘라냈다. 그 자리에는
  **실제 법인명**도 온다("회계법인 지평"의 "지평") — 주소 잔재 한 글자가 공백 없이
  앞에 붙은 "…4층회계법인 지평"이 "층회계법인"으로 뭉개져 ① 같은 감사인을 교체로
  오판(실측 id 5965: "층회계법인 지평" 3개 연도 vs "회계법인 지평" 1개 연도)하고,
  ② 오염 접두어가 같은 서로 다른 두 법인("층회계법인 지평"/"층회계법인 상지원")이
  같은 키가 돼 **실제 교체를 조용히 놓칠** 수도 있었다. 수정: 접미어 뒤는
  `_AUDITOR_SIGNER_TOKENS`가 **실제로 나올 때만** `_cut_at_signer()`로 자르고,
  법인명이 남으면 살린다. 접미어 앞뒤 모두에 글자가 남으면 어느 쪽이 법인명인지
  단정할 수 없어 **아무것도 버리지 않는다** — 단 **앞이 한 글자면 주소 잔재로 보고**
  접미어-선행 표기로 정규화한다(파서 `auditor.py::_clean_name`의 "숫자 뒤 한 글자는
  주소 잔재" 가드와 같은 관행. 공백이 있으면 그 가드가 이미 걷어내므로 여기 오는
  것은 공백 없이 붙은 경우뿐이고, 정상 법인명은 두 글자 이상이라 안 깎인다).
  **파서는 여전히 무변경**이고 DB에는 원문 표기 그대로 저장한다. 로컬 캐시 재스캔
  (3,699 스냅샷/1,159개 회사, 읽기 전용·API 0건): 키가 바뀌는 이름은 2종 11건
  ("층회계법인 상지원" 8 → "상지원회계법인", "층회계법인 지평" 3 → "지평회계법인")
  이고 **회사 판정이 바뀌는 것은 id 5965 1건뿐**(변동 있음→변동 없음, 최종 분포
  변동 있음 222 / 없음 745 / 판정 불가 192). 잔여 한계: 접미어 **앞**이 오염된
  "층신한회계법인"류(실측 1건, 그 회사는 연도 1개뿐이라 판정 불가)는 이 규칙으로
  못 고친다 — 법인명 사전 없이는 구분 불가라 그대로 둔다.
  · **[보정 2] 연도별 변경 판정을 서버가 내려준다** (2026-07-26 dart-qa 리뷰 →
  설계 변경): 프론트가 `ResultDetailDrawer.tsx`에서 자체 `auditorKey()`(공백 제거만)
  로 연도별 뱃지를 그려 백엔드 `_auditor_key()`와 답이 갈렸다(실측 3건 id 5562/
  6537/6569 — 대표이사 교체를 감사인 교체로 오판해 **목록 컬럼은 "동일"인데 상세는
  "변경"**). 정규화를 두 곳에 두지 않도록 `GET .../history` 응답
  (`FinancialSnapshotResponse`)에 **`auditor_changed_from_prev: bool | None`**을
  추가했다 — 그 연도의 `auditor_name`과 **그 이전에 이름이 확보된 가장 최근 연도**의
  이름을 (보정 1이 반영된) `_auditor_key()`로 비교한 값이고, 둘 중 하나라도 없으면
  `null`(판정 불가)이다. 계산은 순수 함수 `pipeline.auditor_change_flags(names)`
  (연도 오름차순 목록 → 플래그 목록)로 분리했고 API는 이력 조회 시 그 자리에서
  호출한다(DB 컬럼 추가 없음, 저장하지 않음 — 정규화 규칙이 바뀌면 즉시 반영된다).
  전수 대조: 1,159개 회사 전부에서 "연도별 플래그에 True가 하나라도 있다" ==
  "`auditor_changed`=1"이 성립(모순 0건). **프론트는 이 필드만 읽으면 되고 자체
  `auditorKey()`는 제거 대상**(dart-frontend 몫).
  · **[보정 3] 파서 추출 단계의 앞글자 절단 버그**(`auditor.py::_clean_name`,
  2026-07-26 dart-qa 리뷰 → dart-parser 수정, [보정 1]과는 별개 계층 — 이건 파서가
  DB에 저장하는 값 자체의 오류다): 주소와 이름이 한 줄에 붙는 서식에서 "숫자 뒤
  남은 주소 단위 글자"를 떼어내는 옛 가드가 "숫자 뒤 아무 1글자 토큰"이면 다
  발동해서, 글자 사이가 벌어진 법인명이 주소 뒤에 바로 붙으면("…한강대로 100삼
  일 회 계 법 인") 이름의 **진짜 첫 글자**까지 주소 단위로 오인해 삭제했다
  ("일회계법인" — 실제는 "삼일회계법인"). 캐시 4,923건 전수 스캔: 135건 중
  59건(1.2%)이 이렇게 손상, 76건은 의도대로 "층"/"호" 제거. 수정:
  `_ADDRESS_UNIT_CHARS = frozenset("층호")`로 **실측된 1글자 단위만** 허용(다른
  후보 "동"/"가"는 "동원회계법인"/"가율회계법인" 등 259건과 충돌해 제외 — 새 글자는
  실측 사례가 나왔을 때만 추가). 이어서 지번 주소 "…1468번지 태흥공인회계사감사반"
  (5건)처럼 **2글자 단위**("번지")도 같은 방식으로 오탐(`번지태흥공인회계사감사반`)을
  냈던 것을 `_ADDRESS_UNIT_WORDS = ("번지",)`로 추가 — 통짜 2글자 매치 + 단위 뒤
  공백이 이어질 때만 떼어내는 이중 가드("지성/지암/지우회계법인"·"지음공인회계사감사반"
  29건과 "100번지성회계법인"처럼 공백 없이 붙어 경계가 모호한 캐시 0건 케이스를
  보존하기 위함). **파서 외 비교 로직(`pipeline.py::_auditor_key()`)은 무변경**,
  DB엔 원문 표기 그대로 저장. 회귀 fixture 6건(20230504000483/20250331002349/
  20250428000005·대조군/20230403002788/20240403003225/20240828000306).
  · 사전 영향 스캔(읽기 전용): 기존 `financial_snapshots` 4,147행 중
  **3,685행이 감사인 이름을 채울 수 있고**(448행은 전기 유래라 대상 아님, 14행은
  원문에 감사인 없음 — 진성 결측), 이력 보유 1,159개 회사 기준 최종 판정(보정
  1~3 모두 반영)은 **변동 있음 221 / 변동 없음 746 / 판정 불가 192**가 될
  전망이었다(실제 실행 결과는 220/747/192 — 아래 소급 반영 기록 참고).
  기능 도입 당시에는 관행대로 신규 Phase 2 실행분부터만 채워지게 두었고
  (**DB의 `auditor_name`/`auditor_changed`가 전부 NULL이라 위 보정 1~3에 해당하는
  소급 처리 대상 자체가 없었다**), 단 `results.auditor_name`은 이 기능 이전부터
  있던 컬럼이라 예외 — 1,206행 중 **15행이 [보정 3]의 절단 버그로 이미 손상
  저장**돼 있었다(우회계법인→일우회계법인 6건, 번지태흥공인회계사감사반→
  태흥공인회계사감사반 1건 등). `pytest` 314 passed(보정 1~3 포함).
  · **기존 완료 Job 결과에 대한 소급 반영 완료(2026-07-26, 사용자 명시 승인
  일회성 예외)**: `backend/scripts/reparse_local_cache.py`(무변경 재사용,
  `--dry-run` 지원, API 호출 0건)로 results 1,211건(rcept 보유+파싱완료, 캐시
  결측 0) 재파싱 → **15행 순수 auditor_name/address 교정**(다른 값 필드
  `val_changed=[]` 전부, parse_status 전환 0건 — OK→OK/PARTIAL→PARTIAL만).
  `--verify` 항등식 자체검증: 부호 오분류 0, DB 드리프트 0(멱등), 크기 불일치
  2건은 기존 "-"→cogs=0 버그(20230410002954/20230406001585)로 이번 작업과 무관.
  실행 전 `dart_search.db.bak.pre_auditor_name_backfill.20260726_195103`로 백업.
  · **연도별 감사인/감사인 변동 자체의 소급 반영 완료(2026-07-26, 사용자 명시
  승인 일회성 예외)**: 위 `results.auditor_name` 교정과 달리 이번 기능이 새로
  만든 두 컬럼(`financial_snapshots.auditor_name` / `results.auditor_changed`)을
  기존 완료 Job에 채운 작업이다. 전용 스크립트
  **`backend/scripts/backfill_auditor_names.py`**(신규 —
  `reparse_financial_snapshots.py`는 "non_operating 2컬럼만"이라는 좁은 책임과
  당기/전기 판정 로직을 갖고 있어 확장하지 않고 분리했다. 감사인은 기간 판정이
  필요 없고(`from_current_period=1` 행만 대상) 대신 회사 단위 집계라는 다른
  책임이 붙는다. `--dry-run`/`--verify` 지원, 재실행 멱등, **API 호출 0건**).
  파싱·비교 로직은 전부 파이프라인 것을 재사용한다 —
  `_safe_extract_auditor_name()`(STEP7과 동일 경로) / `_update_auditor_change_flag()`
  / `_auditor_key()` / `auditor_change_flags()`. 대상 3,699행
  (`from_current_period=1 AND auditor_name IS NULL`, 캐시 결측 0·파싱 예외 0·
  비XML 0) → **3,685행 NULL→값 채움**, 나머지 **14행은 원문에 감사인 서명란이
  없는 진성 결측**(사전 스캔 예측치와 정확히 일치). 이어서 이력 보유 1,159개
  회사의 `results.auditor_changed`를 갱신: **변동 있음 220 / 변동 없음 747 /
  판정 불가 192**(사전 스캔 예측 221/746/192 대비 1개 회사가 "있음→없음"으로
  다름 — 예측 스캔은 별도 임시 스크립트였고, 실제 값이 현재 파이프라인 함수를
  그대로 쓴 판정이다). `--verify` 자체검증(백업본 ATTACH 후 전 컬럼 대조):
  **financial_snapshots의 auditor_name 외 전 컬럼 드리프트 0행**(행 수 4,147
  불변, 전이는 NULL→값 3,685 / NULL→NULL 462뿐, 값→NULL·값→다른 값 회귀 0,
  `from_current_period=0`인데 이름이 채워진 설계 위반 0), **results의
  auditor_changed 외 전 컬럼 드리프트 0행**(행 수 3,421 불변), **연도별
  `auditor_change_flags()`와 회사 단위 캐시의 모순 0건**. 재실행 시 대상 14행
