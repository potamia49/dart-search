...
interface ColumnToggleProps {
  ...
  /** 토글 상태와 무관하게 화면이 강제로 표시 중인 컬럼(예: 휴면·폐업 추정을 목록에
   * 포함시켰을 때의 최근 공시일자). 체크박스를 실제 표시 상태와 일치시켜(체크됨·비활성)
   * 시각적 모순을 없앤다(2026-07-22 디자인 리뷰 반영). */
            ...
            {allColumns.map((col) => {
              ...
              return (
                <Checkbox
                  key={col.key}
                  size="sm"
                  label={forced ? `${col.label} (현재 필터에서 항상 표시)` : col.label}
